...
public class Model {
	...
	public String creaGrafo(String city, Year anno) {
		...
		*/
		...
		List<Adiacenze> archi = dao.calcolaArchi(city, anno) ;
		for(Adiacenze arco : archi) {
			Graphs.addEdge(this.grafo,
					this.verticiIdMap.get(arco.getId1()),
					this.verticiIdMap.get(arco.getId2()), 
					arco.getPeso()) ;
		}
	...
	List<Business> percorsoMigliore(Business partenza, Business arrivo, double soglia) {
	...
	private void cerca(List<Business> parziale, int livello, Business arrivo, double soglia) {
		Business ultimo = parziale.get(parziale.size()-1) ; //se ultimo elemento è uguale a arrivo
		...
		for(DefaultWeightedEdge e: this.grafo.outgoingEdgesOf(ultimo)) {
			if(this.grafo.getEdgeWeight(e)>soglia) {
				...
				Business prossimo = Graphs.getOppositeVertex(this.grafo, e, ultimo) ; //vertice opposto a ultimo usando arco e 
